...
class ViewController: GLKViewController, ARSessionDelegate, RTABMapObserver, UIPickerViewDataSource, UIPickerViewDelegate, CLLocationManagerDelegate {
    ...
    private func updateState(state: State)
    {
        ...
        // 조건에 따라 "First-P. View" 항목 추가
        if self.mState == .STATE_CAMERA || self.mState == .STATE_MAPPING || self.mState == .STATE_VISUALIZING_CAMERA {
            children.append(UIAction(title: "First-P. View", image: cameraMode == 0 ? UIImage(systemName: "checkmark.circle") : UIImage(systemName: "circle"), handler: { _ in
        ...
        // 최종 메뉴 생성
        let cameraMenu = UIMenu(title: "View", options: .displayInline, children: children)
