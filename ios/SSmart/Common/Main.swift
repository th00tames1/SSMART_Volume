...
class MapViewController: UIViewController, CLLocationManagerDelegate, UIGestureRecognizerDelegate {
    ...
    func setupeditcoordsButton() {
        ...
        editcoordsButton.backgroundColor = .systemGreen // 배경 색상을 녹색으로 변경
        editcoordsButton.setImage(UIImage(systemName: "globe"), for: .normal) // 아이콘을 globe로 설정
        editcoordsButton.tintColor = .white // 아이콘 색상을 흰색으로 설정
        editcoordsButton.layer.cornerRadius = 30 // 버튼을 원형으로 만들기 위해 반지름을 절반으로 설정 (60x60 버튼 기준)
        ...
        NSLayoutConstraint.activate([
            editcoordsButton.widthAnchor.constraint(equalToConstant: 60), // 크기를 60으로 설정
            editcoordsButton.heightAnchor.constraint(equalToConstant: 60), // 크기를 60으로 설정
