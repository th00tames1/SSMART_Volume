...
//        orthoGridSlider.setValue(180, animated: false)
    ...
    func showProjectSelectionPopup(cancel: Bool) {
        ...
        let cancel = cancel
        let alert = UIAlertController(title: "Select Project", message: "Please create or select a project file", preferredStyle: .alert)

        let newProjectAction = UIAlertAction(title: "New Project", style: .default) { _ in
            self.showNewProjectNameInputPopup()
        }

        let browseProjectAction = UIAlertAction(title: "Browse Project", style: .default) { _ in
            self.showBrowseProjectPopup()
        }
        
        let editProjectAction = UIAlertAction(title: "Edit Project", style: .default) { _ in
                ...
                if let databasePath = self.openedDatabasePath {
                    ...
                    if fileExists {
                        ...
                        if(self.mLastKnownLocation != nil)
                        {
                            ...
                            gpsString = "GPS: [not available111]\n"
                        }
                    }
                }
                ...
                if let oldDBPath = self.openedDatabasePath {
                    ...
                    if FileManager.default.fileExists(atPath: oldCSVFilePath.path) {
                        ...
                        do {
                            ...
                            // rename에 실패했더라도 아래 saveCSV로 새로운 CSV를 생성할 수 있음
                        }
                    }
                }
...
//            self.optimization(pointcloud: true, approach: -1)
    ...
    func openDatabase(fileUrl: URL) {
        ...
        DispatchQueue.background(background: {
            ...
            self.dismiss(animated: true)
            if(status == -1) {
                self.showToast(message: "맵이 로드되었으나 그래프 최적화에 실패하여 맵을 표시할 수 없습니다. 그래프 최적화 접근 방식을 변경하거나 \"Settings -> Mapping...\"에서 그래프 최적화를 활성화/비활성화한 후 다시 시도하세요.", seconds: 4)
            }
            else if(status == -2) {
                self.showToast(message: "데이터베이스 열기에 실패했습니다: 메모리가 부족합니다! 설정에서 포인트 클라우드 밀도를 낮춘 후 다시 시도하세요.", seconds: 4)
            }
            else {
                if(status >= 1 && status<=3) {
                    ...
                    self.setGLCamera(type: 2)
                    self.updateState(state: .STATE_IDLE)
                    self.showToast(message: "데이터베이스가 로드되었습니다.", seconds: 2)
                }
...
//        rtabmap!.setGridRotation((Float(sender.value)-90.0)/2.0)
    ...
    @IBAction func cropButtonTapped(_ sender: UIButton) {
        ...
        if isCropping {
            self.showToast(message: "Cropping ON", seconds: 1)
            cropButton.tintColor = .systemRed
        } else {
            self.showToast(message: "Cropping OFF", seconds: 1)
    ...
    @IBAction func editsaveButtonTapped(_ sender: UIButton) {
        //Volume Calculation
        self.showToast(message: "The volume has been calculated.", seconds: 2)
        ...
        do {
            ...
            if FileManager.default.fileExists(atPath: csvFilePath.path) {
                do {
                    ...
                    print("CSV 파일 \(csvFilePath) deleted")
                } catch {
